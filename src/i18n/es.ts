export default {
  about: {
    title: "SOBRE MÍ",
    p1: "Soy un desarrollador web bilingüe con 3 años de experiencia.",
    p2: "Me encanta crear interfaces limpias, intuitivas y equilibradas entre diseño y funcionalidad. Siempre empiezo en Figma, donde prototipo y experimento antes de convertir mis ideas en páginas web creadas con React, Next.js y Astro.",
    p3: "Todo lo que ves en esta página, incluidas las ilustraciones, lo he diseñado yo mismo con Figma.",
  },

  footer: {
    madeBy: "Creado con ❤️ por Álvaro Alarcón",
    backToTop: "Volver arriba",
  },

  intro: {
    title: "NO VAMOS A PERDER EL TIEMPO",
    subtitle: "Si buscas un desarrollador web que sabe...",
    punchline: "¡ESE SOY YO!",
    pitch:
      "y 3 años construyendo y desplegando web apps, desde el diseño hasta el lanzamiento: restaurantes, redes sociales, webs para bodas y más.",
    buttons: {
      learnMore: "MÁS INFO",
      contactMe: "CONTÁCTAME",
      myResume: "VER MI CV",
    },
  },

  projects: {
    title: "PROYECTOS",
    visitWebsite: "VISITAR WEB",
    readBlog: "SABER MÁS",
    watchDemo: "VER DEMO",
  },
  digitalBadges: {
    title: "INSIGNIAS DIGITALES",
  },
  blogs: {
    title: "BLOG",
    readMore: "LEER POST",
  },

  contact: {
    title: "CONTACTO",
    form: {
      name: "Tu nombre",
      placeholderName: "P. ej. Álvaro Alarcón",
      email: "Tu email",
      placeholderEmail: "P. ej. nombre@dominio.com",
      message: "Tu mensaje",
      placeholderMessage: "Escribe aquí todo lo que quieras",
      CTA: "Enviar",
      autoresponse:
        "¡Hola! Gracias por ponerte en contacto. Revisaré tu mensaje lo antes posible. ¡Que tengas un buen día! Álvaro",
    },
    validation: {
      nameMinLength: "❗El nombre debe tener más de 3 caracteres.",
      emailInvalid: "❗Dirección de correo electrónico inválida",
      messageMinLength: "❗El mensaje debe tener más de 10 caracteres",
      invalidEmail: "Por favor, introduce un correo electrónico válido.",
      serverError: "No pudimos conectarnos al servidor. Por favor, inténtalo de nuevo más tarde.",
    },
  },
};
